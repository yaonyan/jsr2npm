...
export async function generatePackageJson(
  ...
  // Simply filter out @jsr/* dependencies
  const deps = (originalPkg.dependencies || {}) as Record<string, unknown>;
  const npmDeps = filterNpmDependencies(deps);
...
function filterNpmDependencies(deps: Record<string, unknown>): Record<string, string> {
...
async function readOriginalPackageJson(packageDir: string): Promise<PackageJson> {
  try {
...
function buildNewPackageJson(
  ...
  // Start with original package.json (already has all metadata from JSR)
  const newPkg = { ...originalPkg };

  // Remove JSR-specific and unnecessary fields
  delete newPkg._jsr_revision;
  delete newPkg.devDependencies;
  delete newPkg.scripts;
  delete newPkg.files;

  // Set dependencies (filtered npm deps only)
  newPkg.type = "module";
